...
func _physics_process(delta: float) -> void:
	...
	if direction != 0:
		...
		visuals.scale.x = facing_dir  # +1 right, -1 left
	...
	faceSprite.play("FaceTalk" if Input.is_action_pressed("interact") else "FaceIdle")
	dogFace.play("Talk" if Input.is_action_pressed("interact") else "Idle")
